...
- Real-time via WebSocket when available; automatic fallback to polling.
- WS priority: --ws override > config ws_endpoints > local ws://127.0.0.1:9944 > finney fallback (if network=finney).
...
def ginput(prompt_text: str) -> str:
    ...
    try:
        # Print prompt and flush, then call input() without its own prompt
        sys.stdout.write(prompt)
        sys.stdout.flush()
        return input()
    except EOFError:
...
# -------- WebSocket listener with local+Finney fallback and --ws override --------
def start_event_listener(cfg, ws_override: str | None = None):
    # Build prioritized endpoints:
    # 1) explicit CLI override
    # 2) cfg["ws_endpoints"]
    # 3) local node
    # 4) finney fallback (only if network == finney)
    endpoints = []

    if ws_override:
        endpoints.append(ws_override)

    if cfg.get("ws_endpoints"):
        endpoints.extend(cfg["ws_endpoints"])

    if "ws://127.0.0.1:9944" not in endpoints:
        endpoints.append("ws://127.0.0.1:9944")

    if str(cfg.get("network", "")).lower() == "finney":
        if "wss://finney.subtensor.ai:443" not in endpoints:
            endpoints.append("wss://finney.subtensor.ai:443")
...
def run():
    ...
    # Optional WS override: --ws wss://host:port
    ws_override = None
    if "--ws" in sys.argv:
        try:
            ws_override = sys.argv[sys.argv.index("--ws") + 1]
        except Exception:
            log("Flag --ws given but no endpoint provided; ignoring.")
            ws_override = None
    ...
    if poll_only:
        ...
        threading.Thread(target=start_event_listener, args=(cfg, ws_override), daemon=True).start()
...
if __name__ == "__main__":
    if len(sys.argv) < 2 or sys.argv[1] not in ["setup","run"]:
        print("Usage: python3 taoplicate.py setup|run [--pm2|--no-pm2] [--summary-now] [--dry-run] [--poll-only] [--no-poll] [--debug] [--once] [--ws <wss://...>]")
        sys.exit(1)
    if sys.argv[1] == "setup":
        setup()
    else:
