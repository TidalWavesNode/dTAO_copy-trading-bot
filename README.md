...
- After setup, optionally start with pm2 automatically.
"""
...
# ----------------- PM2 helper -----------------
def maybe_start_with_pm2():
    """Ask whether to run now under pm2; start if yes."""
    print()
    yn = (ginput("Start TAOplicate now with pm2? [y/N]: ").strip().lower() or "n")
    if yn != "y":
        gprint("Setup complete. You can later run:  pm2 start taoplicate.py --name TAOplicate --interpreter python3 -- run")
        return
...
def setup():
    ...
    # Immediate weight explainer with a basic example
    print()
    ...
    # Ask to start with pm2 now
    maybe_start_with_pm2()
